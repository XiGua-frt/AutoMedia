...
更完整的产品说明、接口与协议见 **[docs/产品技术文档.md](docs/产品技术文档.md)**（下文与仓库实现不一致时，以 **代码与 `python-backend/docker-compose.yml`** 为准）。
...
| 智能体 | 阶段 | 职责 |
|--------|------|------|
| RAG 检索（Agent 0） | 标题前（隐式） | 按主题与风格检索知识片段，写入状态供后续 Prompt 使用 |
...
国内镜像拉取困难时，可尝试同目录下的 `docker-compose.mirror.yml`（若存在）。
